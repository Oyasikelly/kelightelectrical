...
export default function HomeSection() {
	...
	return (
		<section
			...
			{/* Sliding Images */}
			<div className="w-full h-[60vh] max-w-7xl mx-auto relative overflow-hidden">
				{/* Image + caption */}
				...
				<AnimatePresence mode="sync">
					<motion.div
						key={`caption-${slides[currentSlide].id}`}
						className="absolute inset-0 flex items-center justify-center bg-black/40 text-white text-2xl font-semibold"
						initial={{ opacity: 0 }}
				...
				<button
					...
					className="absolute left-4 top-1/2 -translate-y-1/2 bg-gray-800/60 text-white p-2 rounded-full">
					&#8249;
				</button>
				<button
					onClick={() => setCurrentSlide((prev) => (prev + 1) % slides.length)}
					className="absolute right-4 top-1/2 -translate-y-1/2 bg-gray-800/60 text-white p-2 rounded-full">
					&#8250;
				</button>
			</div>
			...
			<div className="bg-blue-400 py-12 text-center text-white mt-16 flex flex-col items-center justify-center">
				<div className="max-w-7xl">
					<motion.h3 className="text-3xl font-semibold mb-4">
						Ready to Start Your Project?
					</motion.h3>
					<p className="text-xl mb-6">
						Let’s discuss your electrical needs today! Get a free quote and
						expert advice.
					</p>
					<motion.a
						href="/contact"
